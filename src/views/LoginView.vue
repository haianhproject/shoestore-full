        ...
        <div class="card border-0 shadow-lg rounded-4 p-4 animate-fade-in">
          ...
          <form @submit.prevent="handleLogin">
            <div class="mb-3">
              ...
              <div class="input-group">
                ...
                <input
                  type="text"
                  v-model="username"
                  class="form-control bg-light border-start-0 rounded-end-pill shadow-none"
            ...
            <div class="mb-3">
              ...
              <div class="input-group">
                ...
                <input
                  ...
                  v-model="password"
                  class="form-control bg-light border-x-0 shadow-none"
                  placeholder="••••••••"
                  required
                >
                <span
                  class="input-group-text bg-light border-start-0 rounded-end-pill cursor-pointer text-secondary"
            ...
            <div class="text-end mb-4">
              <router-link to="/forgot-password" class="text-warning small fw-bold text-decoration-none">
                Quên mật khẩu?
              </router-link>
            </div>
...
const handleLogin = async () => {
  ...
  try {
    // 1. Tìm email tương ứng với Username trong Firestore
    const q = query(collection(db, "users"), where("username", "==", username.value.trim()));
    const querySnapshot = await getDocs(q);

    if (querySnapshot.empty) {
      throw new Error("Tên đăng nhập này không tồn tại!");
    }
    ...
    // 3. Lưu thông tin vào localStorage để Navbar hiển thị
    ...
    // 4. Phát sự kiện để Navbar cập nhật lại ngay lập tức
    ...
    // Việt hóa các thông báo lỗi từ Firebase
    if (error.message === "Tên đăng nhập này không tồn tại!") {
      ...
      errorMsg.value = "Lỗi kết nối mạng. Vui lòng kiểm tra lại.";
    } else {
